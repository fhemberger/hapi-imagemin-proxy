...
exports.get = (image) => (new Promise((resolve, reject) => {

    if (internals.isLocal(image)) {
        return Fs.readFile(image, (err, payload) => err ? reject(err) : resolve(payload));
    }

    Wreck.get(image, (err, response, payload) => err ? reject(err) : resolve(payload));
}));
