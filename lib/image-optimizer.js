...
internals.getSrcImage = function (srcImage, callback) {
    ...
    if (!/^https?:\/\//.test(srcImage)) {
        return callback(null, gm(srcImage).noProfile());
    }

    Wreck.get(srcImage, (err, response, payload) => {

        if (err) {
            return callback(err);
        }

        return callback(null, gm(payload).noProfile());
    });
};
...
internals.convertImage = function (options, callback) {
    ...
    const srcImageName = internals.resolveFilePath(options.source, options.filename);

    internals.getSrcImage(srcImageName, (err, destImage) => {
        ...
        destImage.size((err) => {
            ...
            if (err) {
                ...
                return callback(new LoadImageError(`Error loading ${srcImageName}: ${err.message}`));
            }
            ...
            destImage.toBuffer(options.format, (err, buffer) => {
                ...
                if (err) {
                    ...
                    return callback(new ResizeImageError(err.message));
                }
...
internals.optimizeImage = function (format, imageBuffer, callback) {
    ...
    stream2buffer(childProcess.stdout, (err, buffer) => {
        ...
        if (err) {
            ...
            return callback(new OptimizeImageError(err.message));
        }
